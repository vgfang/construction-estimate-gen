...
import { DEFAULT_DISCLAIMER } from "../utils/storage";
...
export function SettingsModal({
  ...
  const patch = (part: Partial<Company>) =>
    onCompanyChange({ ...company, ...part });
          ...
          <div className="mt-4 flex items-center gap-3">
            {company.logo ? (
              <img
                src={company.logo}
                alt="Logo"
                className="h-16 w-16 rounded border border-slate-200 bg-slate-50 object-contain"
              />
            ) : (
        ...
        <section className="border-t border-slate-200 pt-5">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="relative">
              <TextArea
                label="Disclaimer text (printed on estimate)"
                value={company.disclaimer ?? DEFAULT_DISCLAIMER}
                onChange={(v) => patch({ disclaimer: v })}
                rows={3}
              />
              {company.disclaimer !== DEFAULT_DISCLAIMER && (
                <button
                  className="mt-1 text-xs text-slate-400 underline hover:text-slate-600"
                  onClick={() => patch({ disclaimer: DEFAULT_DISCLAIMER })}
                >
                  Reset to default
                </button>
              )}
